// ============================================================
// STORAGE — D1 Database schema + KV session store
// ============================================================
...
-- Request Traces (observability spine)
...
`,
  seed_skills: `
INSERT OR IGNORE INTO skill_packages (id, name, trigger_pattern, system_fragment, tools, priority, token_budget, enabled, metadata, created_at)
VALUES 
  ('general','General Assistant','.','You are a helpful, precise, and memory-aware assistant.','[]',0,200,1,'{"builtin":true}','${new Date().toISOString()}'),
  ('code','Code Assistant','\\b(code|function|debug|implement|typescript|javascript|python|sql|bug|error)\\b','You are also an expert software engineer. Write clean, typed, commented code.','[]',10,300,1,'{"builtin":true}','${new Date().toISOString()}'),
  ('research','Research Mode','\\b(research|analyze|compare|explain|why|how does|what is)\\b','You are a thorough researcher. Structure answers clearly. State confidence levels.','[]',5,250,1,'{"builtin":true}','${new Date().toISOString()}'),
  ('memory_aware','Memory-Aware Mode','\\b(remember|recall|earlier|before|last time|previously)\\b','The user is referencing prior context. Reference retrieved memories explicitly.','[]',15,150,1,'{"builtin":true}','${new Date().toISOString()}');
`,
}
...
export class StorageService {
  ...
  async initialize(): Promise<void> {
    // Run each DDL statement individually (D1 doesn't support multi-statement exec)
    const ddlStatements = [
      `CREATE TABLE IF NOT EXISTS memory_items (
        id TEXT PRIMARY KEY,
        type TEXT NOT NULL CHECK(type IN ('episodic','semantic','summary')),
      ...
      `INSERT OR IGNORE INTO skill_packages (id, name, trigger_pattern, system_fragment, tools, priority, token_budget, enabled, metadata, created_at) VALUES ('general','General Assistant','.','You are a helpful, precise, and memory-aware assistant.','[]',0,200,1,'{"builtin":true}','2026-01-01T00:00:00Z')`,
      `INSERT OR IGNORE INTO skill_packages (id, name, trigger_pattern, system_fragment, tools, priority, token_budget, enabled, metadata, created_at) VALUES ('code','Code Assistant','\\b(code|function|debug|implement|typescript|javascript|python|sql|bug|error)\\b','You are also an expert software engineer. Write clean, typed, commented code.','[]',10,300,1,'{"builtin":true}','2026-01-01T00:00:00Z')`,
      `INSERT OR IGNORE INTO skill_packages (id, name, trigger_pattern, system_fragment, tools, priority, token_budget, enabled, metadata, created_at) VALUES ('research','Research Mode','\\b(research|analyze|compare|explain|why|how does|what is)\\b','You are a thorough researcher. Structure answers clearly. State confidence levels.','[]',5,250,1,'{"builtin":true}','2026-01-01T00:00:00Z')`,
      `INSERT OR IGNORE INTO skill_packages (id, name, trigger_pattern, system_fragment, tools, priority, token_budget, enabled, metadata, created_at) VALUES ('memory_aware','Memory-Aware Mode','\\b(remember|recall|earlier|before|last time|previously)\\b','The user is referencing prior context. Reference retrieved memories explicitly.','[]',15,150,1,'{"builtin":true}','2026-01-01T00:00:00Z')`,
    ]
    ...
    for (const sql of ddlStatements) {
      try {
        ...
        if (!e.message?.includes('already exists') && !e.message?.includes('duplicate')) {
          console.error('Migration error:', e.message?.slice(0, 100))
        }
      }
    }
  }
  ...
  async upsertSession(session: any): Promise<void> {
    await this.db
      .prepare(`
        INSERT INTO sessions (session_id, user_id, project_id, token_budget, tokens_used, 
      ...
      .prepare(`
        INSERT OR REPLACE INTO request_traces 
          (trace_id, session_id, user_id, request_at, completed_at, stage_log,
           retrieval_candidates, thalamus_scores, dropped_context, token_breakdown,
           consolidation_queued, error)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
      .bind(
        trace.trace_id,
  ...
  async listTraces(params: { session_id?: string; user_id?: string; limit?: number }): Promise<any[]> {
    ...
    await this.db
      .prepare(`
        INSERT INTO consolidation_jobs 
          (job_id, session_id, trace_id, status, transcript, context_used, insula_permissions, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `)
      .bind(
        job.job_id,
  ...
  async updateJobStatus(job_id: string, status: string, result?: any, error?: string): Promise<void> {
    await this.db
      .prepare(`
        ...
        WHERE job_id = ?
      `)
      .bind(status, result ? JSON.stringify(result) : null, error ?? null, new Date().toISOString(), job_id)
      .run()
  }
  ...
  // ── API Keys ───────────────────────────────────────────────
  async validateApiKey(keyHash: string): Promise<{ user_id: string; rate_limit_rpm: number } | null> {
    const row = await this.db
